...
def derive(wallpaper):
    ...
    out = {k: hex_of(harmonize(parse_hex(v), target)) for k, v in BASE.items()}
    return out, target
...
def write_alacritty(p, n, wallpaper):
...
def write_ghostty(p, n, wallpaper):
...
def main(argv):
    ...
    for name, writer in WRITERS.items():
        with open(os.path.join(out, name), "w") as f:
            f.write(writer(palette, NEUTRALS, os.path.basename(wallpaper)))
    print(f"harmonized {len(WRITERS)} terminal configs to hue "
          f"{'(none)' if target is None else f'{target:.0f}'} in {out}")
    return 0
